...
fn unsupported_omml_keeps_raw_markup_without_a_projection() {
    // `m:phant` (phantom spacing) is an intentionally-excluded niche construct,
    // outside the projected subset: the OMML is retained verbatim and no typed
    ...
    assert!(math.omml.contains("<m:phant>"));
