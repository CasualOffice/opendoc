...
| M-002 | LibreOffice differential fidelity harness | Claude Code | Done | `tools/opendoc-fidelity`: compares importer text vs `soffice --convert-to txt` by word-multiset match %; corpus 6/7 at 100%, footnotes now closed. Round-trip proxy until the Phase-2 writer; not a CI dep. |
...
| P1A-017 | No-edit DOCX writer (round-trip reconstruction) | Claude Code | Done | New `casual-doc-export`: `write_package(&RetainedSource)` reconstructs a valid DOCX byte-identically; makes round-trip end-to-end verifiable; 3 tests. Semantic writer is Phase 2. |
...
| P1B-R5 | Writer paragraph-property completeness | Claude Code | Open (bug) | `write_paragraph_properties` still drops `spacing` (paragraph), `borders` (`w:pBdr`), `shading` (`w:shd`), and `tabs` (`w:tabs`) — a fixed-point break for a paragraph using them (like P1B-R3 for runs). `widow_control` now fixed. Should land as its own slice (the table borders/shading writer helpers are reusable). |
| P1B-005d | Writer media/binary parts | Claude Code | Planned | drawings + embedded fonts (`.odttf`) + note/comment images: the binary-part path with part-name determinism; unblocks font 1A.3 embedded fonts. |
...
| P1B-FONT-1A3 | Embedded fonts | Claude Code | Planned | `.odttf` in-core de-obfuscation preserving `fontKey` (byte-identical re-export) + sfnt validation; `EmbeddedFontSet`; settings embedding flags. |
...
| P1B-R4 | Importer `cstheme` spelling | Claude Code | Open (import gap) | Importer reads `w:csTheme` (capital T), but real OOXML `w:rFonts` uses `w:cstheme` (lowercase); a real doc's cs theme-font slot is missed on import. Writer emits `w:csTheme` to match the importer (round-trips through us). Fix: accept both spellings on import. |
