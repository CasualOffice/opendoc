...
| P1B-FONT-1A2b | theme fontScheme | Claude Code | Next | theme1.xml `a:fontScheme` (major/minor × latin/ea/cs + per-script overrides) wired into `Definitions` so the 8-value `ThemeFontRef` slots resolve; importer `/theme` rel resolution; writer emits `word/theme/theme1.xml` + override + relationship. |
