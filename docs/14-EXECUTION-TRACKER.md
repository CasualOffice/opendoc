...
| P1A-005 | Read path: relationship-based main-document discovery (R1) | Claude Code | In progress | ADR-028 adds `quick-xml`; relax admitter to `[Content_Types].xml` + `_rels/.rels`; discover `officeDocument` relationship (transitional+strict), resolve target, verify content type, fail-closed. |
