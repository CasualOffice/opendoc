...
| P1B-005e2 | Writer headers/footers | Claude Code | Next | header1.xml/footer1.xml… — N parts each with its own r:id referenced from the sectPr `w:headerReference`/`w:footerReference`; reproduce the r:id<->HeaderFooterId correspondence (importer keys by /header rel ORDER). Per-part-rels infra exists; needs pre-assigned extra-part rIds so the sectPr can reference them. |
