...
| P1B-004 | Writer inline constructs | Claude Code | Next | hyperlinks (+ `document.xml.rels` generation + content-type), fields (`fldSimple`), bookmarks (name from `Definitions`, shared `w:id` per `BookmarkId`), revisions (`ins`/`del` + `delText`), inline content controls (`w:sdt`). Drawings/text-boxes (media) and note/comment refs (definition parts) deferred to P1B-005. Closes review finding P1B-R2. |
...
| P1B-FONT | Font management design | Claude Code | Researching | Research-first (owner directive): prior art across Word/OOXML (fontTable, rFonts+theme, embedding/odttf, panose), LibreOffice, ONLYOFFICE, CSS matching; Rust crate stack (fontdb/font-kit/fontique/cosmic-text/swash/fontations/ttf-parser); OpenDoc current state. Deliverable: `40-FONT-MANAGEMENT-DESIGN.md` -> discuss/finalize -> implement (model + round-trip first, resolution later). |
