...
| P1B-005f | Writer text boxes + embedded fonts | Claude Code | Next | text boxes (`w:txbxContent` block content in a drawing shape) reuse the drawing/media path; embedded fonts (font 1A.3) still need additive model work (EmbeddedFontSet/fontKey) before a writer contract. |
