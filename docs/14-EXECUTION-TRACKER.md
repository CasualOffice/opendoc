...
| P1B-005b | Writer numbering/notes/headers/comments | Claude Code | Next | numbering.xml, footnotes/endnotes, headers/footers (+ section refs, N parts w/ per-part r:ids), comments.xml. Contracts being mapped by a parallel agent workflow. |
| P1B-005c | Writer media/binary parts | Claude Code | Planned | drawings + embedded fonts (`.odttf`) + note/comment images: the binary-part path with part-name determinism; unblocks font 1A.3 embedded fonts. |
