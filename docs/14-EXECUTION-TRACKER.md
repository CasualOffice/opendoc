...
| --- | --- | --- | --- | --- |
| M-001 | Decompose god-files into modules | Claude Code | In progress | Owner directive: no god files. Done: `model/v1.rs` (1574→`v1/` module); `casual-doc-import` (1324-line `lib.rs` → `config`/`error`/`report`/`properties`/`styles`/`body`/`lib`/`tests`, ≤302 lines each). Next: model `lib.rs` (v0, 1306), `ooxml/lib.rs` (1990), `sdk/lib.rs` (2161). API preserved via re-exports. |
