...
| P1B-005e | Writer headers/footers | Claude Code | Next | header1.xml/footer1.xml… — N parts each with its own r:id referenced from a section `w:sectPr` `w:headerReference`/`w:footerReference`; reproduce the r:id<->HeaderFooterId correspondence. Per-part-rels infra now exists. |
