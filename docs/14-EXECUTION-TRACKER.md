...
| P1B-FONT-1A2 | fontTable.xml + theme scheme | Claude Code | Next | Typed `FontTable`/`FontDescriptor` (panose/sig/altName/family/pitch) + theme `FontScheme` (+ per-script overrides); importer `/fontTable` + `/theme` rel resolution; writer emits both parts. |
...
| P1B-R3 | Semantic writer run-property completeness | Claude Code | Open (bug) | `write_run_properties` (semantic.rs) emits only b/i/strike/u/color/sz + now rFonts; it SILENTLY DROPS `all_caps`/`small_caps`/`hidden`/`web_hidden`/`double_strike`/`character_spacing`/`kerning`/`position`/`language`/`highlight`/`emphasis`/`vertical_alignment` — a real fixed-point break for a run using them (not yet in the round-trip corpus). Pre-existing from P1B-002. Should land as its own slice before the writer is trusted for arbitrary models. |
