...
| P1B-FONT-1A3 | Embedded fonts (model + import + writer) | Claude Code | Next | additive EmbeddedFontSet/EmbeddedFace on FontDescriptor + settings flags; import reads embed* + fontTable.xml.rels; writer emits .odttf parts VERBATIM (Solution-A pattern) + fontTable.xml.rels. Contract mapped by agent. |
