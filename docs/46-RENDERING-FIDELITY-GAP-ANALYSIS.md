...
| Table style and conditional formatting | Cell background shading now resolves the table-style layer: `style_ref`'s `basedOn` chain contributes each style's base cell/table fill, and the row+cell's combined `cnfStyle` bits select the matching `tblStylePr` conditional region (banding, first/last row/column, corner cells), gated by `tblLook`, in Word's region precedence. Only shading is wired. | Implemented (shading only) by `P1F-TBL-CNF`. **Still open:** the same conditional regions' paragraph/run overrides (e.g. bold header text, region-specific fonts) and table/cell border overrides; `basedOn` inheritance across table styles is exercised but real documents rarely need more than one level. |
...
| Multi-section anchors | Body floats carry source-section provenance through the post-pagination pass; page, margin, physical-side margin, and actual placed-column frames resolve from that section. Header/footer floats use their page's recorded section. | Implemented by P1F-53 for pictures, text boxes, and groups, including continuous sections and nested table cells. Exact character anchoring, mirrored inside/outside parity, and per-section running-content selection remain. |
...
| Text-box overflow | `horzOverflow=clip` and `vertOverflow=clip` bound nested paint on the selected axes; `ellipsis` is preserved and clipped safely. | Partial: exact terminal ellipsis synthesis remains. |
