...
| FID-L-07 | **Floating tables (`w:tblpPr`) render inline** — zero hits in the layout crate. Newsletters, forms, and legal documents use them. | Tables | P1 | M | `grep "tblpPr\|float_position"` over `crates/casual-doc-layout/src` → 0 | Partly fixed — **top-level body** positioned tables now place and wrap (`crates/casual-doc-layout/src/table_float.rs`; `docs/109` row 64). They are lifted out of the galley, resolved through the SAME `wp:anchor` machinery a floating drawing uses, painted and hit-tested on the float layer, and their rectangle feeds the existing `BodyWrapRect` exclusion fixed point. **Still open** (`109` row 64a): positioned tables nested in cells / SDTs / headers / notes, and splitting a tall one across pages |
...
| FID-L-10 | **Watermarks do not appear.** No watermark concept exists anywhere, and a Word watermark is a header shape carrying warped text (`v:textpath` / `a:prstTxWarp`) — neither text-path form is typed, so the text vanishes and only the shape box can paint. | Shapes | P2 | M | `grep -ri watermark crates/` → 0 | **Partly fixed.** A watermark is now a typed section property, imported from Word's `v:textpath`/`v:imagedata` header shape, laid out and painted behind the page, exported back, and authorable. What keeps this open is the row's wider claim: general WordArt text paths are still neither typed nor painted, the DrawingML form (`a:prstTxWarp`) is unrecognised, and a picture watermark's authored scale collapses to Auto because import reads no natural size |
...
| OO-006 | **Hyphenation and advanced drop-cap controls remain** — the original row combined four Layout/References features. | P1 | L | Hyphenation (auto, hyphenate-CAPS, hyphenation zone, consecutive-hyphen limit); line numbers (continuous / restart page / restart section / suppress for paragraph); watermark dialog (text templates, 8 languages, font, semitransparent, diagonal/horizontal, or image with scale); drop cap in-text/in-margin with height-in-rows and full frame settings | **Updated 2026-09-26.** Line numbering is authorable (#609), and watermarks exist end to end. Drop-cap preset authoring now ships on Insert with None/Dropped/In margin, a 1–10 line count, read-back, review gating, and one-step undo. The remaining drop-cap portion is the advanced font/distance/full-frame option set; hyphenation still has no line-breaking implementation. | FID-L-02, FID-L-09, FID-L-10, and the drop-cap row | Partly fixed |
