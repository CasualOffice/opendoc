...
| OO-006 | **No hyphenation, line numbering, watermark, or drop-cap authoring** — four Layout/References features where the *model* mostly exists and the *consumer* does not. | P1 | L | Hyphenation (auto, hyphenate-CAPS, hyphenation zone, consecutive-hyphen limit); line numbers (continuous / restart page / restart section / suppress for paragraph); watermark dialog (text templates, 8 languages, font, semitransparent, diagonal/horizontal, or image with scale); drop cap in-text/in-margin with height-in-rows and full frame settings | **Updated 2026-09-25.** Line numbering is authorable (#609). Watermarks now exist end to end — model, import, layout, render, export, dialog — where the row said they did not; I had earlier recorded the opposite, from a grep that counted comments. Drop caps still **render** without being authorable, and hyphenation still has no line-breaking implementation | FID-L-02, FID-L-09, FID-L-10, and the drop-cap row | Open |
