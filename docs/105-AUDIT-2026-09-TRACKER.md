...
**Last reconciled against `main`: 2026-09-16, after #541.** The counts below are derived
from the rows; a row's Status cell names the PR that closed it so the claim can be checked
rather than trusted.

**Blocking on `main` right now:** FID-L-22 — entering a header paints no caret, so a header
cannot be typed into. Shipped by #538 and being fixed.

Closed this session, with the PR that did it: EV-001…EV-006 (#528), FID-P-02 groundwork and
...
UX-001/UX-002 (#537), UX-010/UX-011 (#541), plus the per-page running-content fix and
...
| UX — editor UI/UX | 24 | 20 | 4 (#537, #541) |
| CQ — engineering quality | 10 | 8 | 2 (#528) |
| FID — fidelity and round-trip | 34 | 27 | 7 (#534, #536, #540, #541) |
...
| CQ-009 | **Design documents no longer describe the implementation.** `63` specifies `--radius` 8px against a shipped 3px; `63` and `64` both specified a four-tab ribbon against five shipped; `64` contradicts itself on horizontal scrolling within one file. A design system that is not the spec of record cannot answer "is this on-system?", which is how the ARIA and radiogroup defects entered. | P2 | S | 3 drifts | Fixed (this PR) |
...
| FID-L-14 | **Emphasis marks, outline, shadow, emboss, imprint, and run borders are modeled and cascaded but unpainted.** Emphasis marks are near-universal in Japanese text; they and run borders are small additions to the decoration pass. | Run format | P2 | S | `crates/casual-doc-layout/src/cascade.rs:469` | Fixed (#541) — emphasis marks and run borders only, drawn as geometry (the bundled faces lack U+3001/U+25CB). outline/shadow/emboss/imprint deliberately still open: they need outline stroking in the raster backend |
