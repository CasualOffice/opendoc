...
## 8. Open questions
1. ~~`fontKey` parsing: confirm plain hex-string reversal (vs .NET mixed-endian) against a real Word-produced `.odttf` fixture in 1A.3.~~ **Closed (FID-L-01).** Verified against a real Word-produced package carrying six embedded faces across three families (Ubuntu regular/bold/italic/bold-italic, Tahoma, Ubuntu Mono): plain hex-string reversal decodes every face to a coherent sfnt header and table directory; .NET `Guid.ToByteArray()` mixed-endian corrupts bytes 8..15 of all six. The two orders agree on the first eight key bytes, so both produce a plausible `0x00010000` signature — an sfnt-version check alone does **not** distinguish them, and the regression guard must compare the full 32-byte prefix (`casual-doc-layout` `font_registry::deobfuscate_odttf`, test `a_real_word_produced_odttf_prefix_decodes_to_its_sfnt_header`).
