...
- A `WordprocessingGroup` that is exactly a group-of-one identity-transform
  ...
  are written as for a floating image, and the shape's solid fill and outline ride
  on the same `style:family="graphic"` automatic style (`draw:fill`/`draw:fill-color`,
  `draw:stroke`/`svg:stroke-color`/`svg:stroke-width`). A shape needs the
