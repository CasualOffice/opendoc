...
| ODT import/export | Later | In progress under docs 94–95. Bounded ODF 1.2–1.4 ODT package/profile admission and deterministic registry detection/dispatch are implemented. The internal semantic importer maps validated core paragraphs, headings, spans, explicit spaces, tabs, and line breaks with deterministic identity, optional exact-source retention, and explicit findings for deferred constructs. Styles, lists, tables, links, notes, bookmarks, media, metadata, edit-tolerant preservation, stable SDK/WASM host integration, export, corpus conformance, and interoperability gates remain incomplete; this is not yet a general ODT support claim or v1 release gate. |
