...
| PDF export | Partial | **Phase 0 shipped and is reachable in the browser** (`crates/casual-doc-pdf`, ADR-031, doc 98): File → Export as PDF transcribes the shared `DisplayList` into a deterministic vector PDF with real selectable/searchable text, embedded TrueType **subset** fonts at the shaper's glyph ids, vector rules/borders/shapes/gradients, and deduplicated image XObjects. The export-only `application.pdf` adapter is registered in the built-in registry and exposed through the generic WASM export seam. Physical Print still uses a raster path. Not covered: live host-font parity on the registry re-pagination path, tagged PDF, hyperlink/outline/destination annotations, PDF/A, encryption, page ranges, CFF subsetting (such faces embed whole), and SVG pictures. Doc 98 §"Implementation status" carries the family-by-family table. |
