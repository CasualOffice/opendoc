...
| ODT import/export | Later | In progress under docs 94–96. Bounded ODF 1.2–1.4 ODT package/profile admission and deterministic registry detection/dispatch are implemented. The internal semantic importer maps validated core paragraphs, headings, spans, explicit spaces, tabs, and line breaks. A deterministic bounded ODF 1.4 writer exports that same core surface, reports unsupported model content, and can return explicitly retained unchanged source bytes exactly. Styles, lists, tables, links, notes, bookmarks, media, metadata, edit-tolerant preservation, stable SDK/WASM host integration, corpus conformance, schema validation, and interoperability gates remain incomplete; this is not yet a general ODT support claim or v1 release gate. |
