...
| REVIEW-GAP-020 | P1 | Partial | The scroll-unification pass removed the per-scroll-frame rebuild (the comment column shares the canvas's single scroll context), and P1G-REVIEW-050 then added the retained item model and offscreen virtualization on top of it. A full render (content edit / resize only) now computes each card's stacked document-scroll position once into `reviewLayout` and mounts only the cards inside a viewport ± overscan band; a scroll runs `mountReviewWindow`, which attaches the cards entering the band and detaches those leaving it without re-parsing the review payload, recomputing geometry, or rebuilding a card. Cards are retained in `reviewCardCache` keyed by a content signature, so an unchanged collapsed card is never rebuilt or re-measured across edits (the active/expanded card and the composer always rebuild so their live controls stay correct), and mounted DOM stays viewport-bounded regardless of review size. Still open: a full render still calls `listComments`/`listRevisions` and recomputes all anchor geometry (only per edit/resize, not per scroll), and there is no formal 100/1,000-item perf-threshold CI benchmark — the new `review-sidebar-virtualization.spec.mjs` asserts the functional bound (offscreen cards detached, remounted on scroll-back) but not a wall-clock budget. | Retain keyed card DOM/state ✓ and virtualize offscreen items ✓ (P1G-REVIEW-050); still add per-model-revision inventory caching to skip the edit-time re-parse, and 100/1,000-item scroll/edit performance gates. |
