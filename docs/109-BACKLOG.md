...
| 4 | HF-162 | Hotfix | The layout is not windowed — `paginate_document` builds a galley and a display list for every page before the first one is shown, so a 1.3M-paragraph file is refused | P1 | L | Open (design in `113`) | New 2026-09-20 (`111` §4 stage 2) | HF-158, HF-077 | The full design is `113-WINDOWED-LAYOUT-DESIGN.md`, which answers the four questions `111` section 4 left open and adds one finding: `viewport_of` / `paginate_viewport` already exist in `incremental.rs` with **zero consumers**, and they window an already-built layout, so they bound paint cost, not memory. Sequenced behind HF-163. |
