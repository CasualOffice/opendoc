...
| 1g | HF-174 | Hotfix | 621 compatibility findings across the corpus report losses that did not happen | P1 | M | Fixed (PR-PLACEHOLDER) | New 2026-09-23 (sweep) | HF-169 | Measured **701**, not 621, once the whole separator-note subtree was counted: over the owner's 15-document corpus the report went **15,769 -> 15,068 occurrences** (the loan agreement 4,306 -> 4,032; the NDA 1,029 -> 997; Sample Contract 1,066 -> 913), and the committed `fixtures/corpus` 213 -> 197. The fix is explicit no-op arms, conditional wherever the no-op is (`<a:effectLst/>` is silent, a populated one is not; `wp14:pctWidth` of `0` is silent, `50000` is not), routed through `Reporter::report`/`report_element` so the class is total. Class, boundary and reasoning: `crates/casual-doc-import/src/noop.rs` and `docs/35` *The no-op class*. Guards: a document that loses nothing reports **zero**, a mirror asserting every conditional member still reports when it carries something, and `the_corpus_reports_exactly_these_findings` pinning all eight committed fixtures to an exact count. Four members of the sweep's list were checked and are NOT no-ops, and stay reported: `w:autoRedefine` (18), `wps:style` (its theme-style children are suppressed, not imported), `a:miter` (6) and `w:docPartUnique` (1). The `a:moveTo` misattribution no longer reproduces — #581's `a:custGeom` arm already owns that subtree. Still open in the same family: an inline text-box shape raises a `drawing` finding of its own. |
...
| 64 | FID-L-07 | Audit | Floating tables (`w:tblpPr`) render inline | P1 | M | Open | 105 §3.2 | — | — |
...
| 94 | FID-L-13 | Audit | Cell `noWrap`, `fitText`, `hideMark` and cell `textDirection` are unconsumed | P2 | M | Open | 105 §3.2 | — | — |
