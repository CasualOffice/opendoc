...
| 2e | HF-179 | Hotfix | The compact toolbar scrolls sideways at every ordinary laptop width, and three of its declared controls were never there at all | P1 | M | Fixed | New 2026-09-23 | UX-004, HF-085 | Reported as "reduce width of style dropdown as well as font size .. its more space than necessary .. and adding scrolling even in normal resolution and scale .. and its better to group things like ordered list and unordered lists and alignments .. like in google docs". Measured on the `rich` fixture in compact mode: at a 1280×800 viewport the bar needed **1413px** against the 1272px it had, and did not stop scrolling until a **1421px** viewport — so it scrolled at 1280, 1366 and 1400, and at 1152 (a 1440px screen at the 125% scale the owner named). `#stylesTrigger` was forced to **227px** by an inline `min-width` in the layout table, on top of a stylesheet rule that had already designed it at 132px; `#fontFamily` was 136px where the ribbon's own authored width is 122px. Underneath the width complaint, three rows of `COMPACT_TOOLBAR` named **context-menu** ids the command registry does not answer — `paragraph.bullets`, `paragraph.numbering`, `comment.add` — and the renderer's `if (!command) continue` dropped them in silence, so the bar shipped with no bulleted list, no numbered list and no Add comment button. The comment above the table claimed a `compact-parity.spec.mjs` was failing the build over exactly that; no such file existed. Fixed: widths cut to 140/122/58px (and the size box loses its spinner, since Docs' − and + sit either side of it), the four alignment buttons become the single align dropdown Google Docs ships, the bar is declared as twelve named `role="group"`s, and whatever does not fit folds into a "⋯" menu — the ribbon's own `updateRibbonOverflow` algorithm — so `overflow-x` is now `hidden` and the bar can never scroll at any width. Every group is inline at any viewport ≥ **1261px**; 1280px leaves 19px of slack. The budget is stated in `docs/115` §8 and guarded at 1280px in `compact-toolbar.spec.mjs`. `main.js` fell 17,801 → 17,658 lines; the ratchet was lowered in the same commit. |
