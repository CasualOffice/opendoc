...
| Native clipboard fidelity | P1 | Copy/cut/paste preserves rich runs, paragraphs, and links; an internal copy also preserves whole tables and list paragraphs (structured block fragment, Editing mode). Plain text remains the fallback. | Copy/cut/paste preserves rich runs, paragraphs, lists, tables, links, and HTML where safe; plain text remains fallback. | Map external HTML tables/lists into the model, add image insertion, and extend structured paste into table cells / cross-document. | Browser clipboard-event tests for formatted text, table cells, links, lists, and plain fallback. |
...
| Command discoverability | P1 | Ribbon, contextual popovers, command palette (⌘⇧P), and one model-aware right-click/Shift+F10 menu exist for prose, links, lists, tables, comments, and suggestions. Palette/menu common actions share descriptors and disabled reasons; the palette now also covers the Review commands and super/subscript that were mouse-only, and shows each command's shortcut. | Word-like commands are discoverable via ribbon, context menu, keyboard shortcuts, command search, and selection toolbar. | Migrate the remaining ribbon and shortcut bindings to the shared descriptors. | Snapshot test of command registry and browser smoke for palette/context invocation. |
| Keyboard shortcuts | P1 | Formatting (⌘B/I/U), find (⌘F), save (⌘S), undo/redo, clipboard/select-all, link authoring (⌘K), add comment (⌘⌥M), review-mode cycle (⌘⇧E), command palette (⌘⇧P), and table Tab handling exist. **⌘K/palette collision resolved** — ⌘K = insert/edit link, palette = ⌘⇧P. | Common Word shortcuts work: save/export, undo/redo, bold/italic/underline, find/replace, select all, copy/cut/paste, links, headings, lists, indent, page break, non-breaking space, table navigation. | Centralize shortcut registry with platform-specific labels and conflict handling. | Unit test registry plus browser shortcut smoke. |
...
| Lists and numbering UX | P1 | Toggle bullet/numbered/**checklist**, nine-level indent (Tab/Shift+Tab + indent controls), Enter-exits-empty-item, restart **and continue** numbering, and **click-to-toggle checkbox items** (engine marker hit-testing) exist. | Bullets/numbering/multilevel lists expose gallery, indent levels, restart/continue, and style-aware numbering. | Remaining: the multilevel/marker-format gallery on top of numbering definitions (checklist authoring is now done — `P1G-LIST-CHECKBOX-029`). | DOCX round-trip tests and browser list workflow smoke. |
...
| Accessibility | P2 | A read-only, model-derived **accessibility tree** (`#a11yDocument`) now mirrors the document's structure — headings as headings (with level), lists as `ul`/`ol`, paragraphs, and tables — off-screen from the engine's `accessibilityTree()` projection, kept in sync on every edit, so a screen reader can read the otherwise-opaque canvas. It is read-only (never a hidden DOM editor). Remaining: live-region status announcements (mode/save/comment), caret-position/reading-cursor sync, table header semantics, and an accessible route for *editing* via AT. | Keyboard users can reach commands; screen readers get document structure/status even though source of truth is model, not DOM. | Add live-region status announcements and caret/selection reading semantics on top of the structural tree. | Axe/static checks plus browser assertion that the exposed tree matches the model (headings/lists/tables). |
...
| 5 | Lists | Core lifecycle now lands: Enter on an empty item exits the list (`P1G-LIST-023`), Tab/Shift+Tab and the indent controls change list level (`P1G-LIST-LEVEL-014`/`P1G-LIST-020`) across all nine generated depths (`P1G-LIST-021`), numbering can be **restarted** (`P1G-LIST-026`) and **continued** (`P1G-LIST-028`), and **checklist** authoring now works (`P1G-LIST-CHECKBOX-029`): a Checklist toolbar toggle creates a checkbox list, the engine draws a `☐`/`☒` marker whose **rect is exposed for hit-testing** (`LayoutSnapshot::marker_rects`, `GlyphRun.is_marker`), and a click on it toggles the item's checked state via one gated edit op (blocked in Viewing/Suggesting like the other list edits). Still open: **multilevel / marker-format picking** (list gallery chrome). | Add the multilevel/marker-format gallery chrome on top of numbering definitions. |
...
| 7 | Clipboard structure | Rich clipboard preserves paragraphs, runs, and links. **Internal** OpenDoc-to-OpenDoc copy/paste now also preserves whole tables and list paragraphs (their numbering) — a structured block fragment reconstructed with fresh ids in Editing mode (`P1G-PASTE-029`). Still flattening: **external** HTML tables/lists (Word/Docs/browser paste still maps to runs), **images** (no insert-image path yet), exotic inline objects inside a copied fragment (drawings/fields/comments dropped), and structured paste in Suggesting mode (no tracked representation — GAP-009) or into a table cell (falls back to flat runs). | Map external HTML tables/lists to the model; add image insertion; extend the fragment into cells and cross-document numbering remap. |
| 8 | Command/shortcut coverage | The ⌘K collision is resolved: **⌘K now inserts/edits a hyperlink** (the Word / Google Docs / Pages convention) and the **command palette moved to ⌘⇧P** (VS Code convention; the header Search pill shows ⌘⇧P). The palette gained the previously mouse-only Review commands (add comment, toggle sidebar, Editing/Suggesting/Viewing mode, next/previous change, accept/reject all) and super/subscript, and now shows each command's keyboard shortcut in its hint column. Remaining: many ribbon/shortcut bindings are still direct listeners rather than one shared registry. | Complete the migration to one registry for ribbon, palette, shortcuts, disabled reasons, and context menus. |
| 9 | Accessibility and touch | **Baseline landed for the a11y half:** a read-only, model-derived off-screen accessibility tree (`#a11yDocument`, from the engine's `accessibilityTree()` projection) now exposes headings/lists/paragraphs/tables to a screen reader and stays in sync with edits, so the canvas document is no longer opaque to AT. Still open: live-region status announcements, caret/reading-cursor semantics, AT-driven editing, and the entire **touch** interaction layer (selection handles, keyboard-safe mobile editing) which remains unbuilt. | Add live-region announcements and caret/selection reading semantics on the structural tree; design a separate touch interaction layer. |
...
5. Command registry: unify palette, shortcuts, menus, contextual commands, and
   ...
   editor context menu are complete in doc 84; the ⌘K/palette shortcut collision
   is resolved (⌘K = link, palette = ⌘⇧P) and the palette now covers the
   Review/super/subscript commands with shortcut hints; the broader
   ribbon/shortcut-to-registry migration remains.
6. Structured clipboard, accessibility/touch, and additional editing surfaces.
   The accessibility baseline — a read-only model-derived structural tree for
   screen readers — has landed; live-region announcements, reading-cursor
   semantics, AT-driven editing, and the touch interaction layer remain.
