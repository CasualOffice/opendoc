...
| Lists and numbering UX | P1 | Toggle bullet/numbered, nine-level indent (Tab/Shift+Tab + indent controls), Enter-exits-empty-item, and restart **and continue** numbering exist. | Bullets/numbering/multilevel lists expose gallery, indent levels, restart/continue, and style-aware numbering. | Remaining: checklist authoring (needs a checked-state model or marker hit-testing) and the multilevel/marker-format gallery on top of numbering definitions. | DOCX round-trip tests and browser list workflow smoke. |
...
| 5 | Lists | Core lifecycle now lands: Enter on an empty item exits the list (`P1G-LIST-023`), Tab/Shift+Tab and the indent controls change list level (`P1G-LIST-LEVEL-014`/`P1G-LIST-020`) across all nine generated depths (`P1G-LIST-021`), and numbering can be **restarted** (`P1G-LIST-026`) and **continued** (`P1G-LIST-028`). Still open: **checklist** authoring (interactive check/uncheck needs a per-item model field or engine marker hit-testing — markers are canvas-rasterized with no click target today) and **multilevel / marker-format picking** (list gallery chrome). | Finish the lifecycle (done); then add the checklist model and gallery/marker-format chrome. |
