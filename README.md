...
| Semantic DOCX import | `.docx` → v1 model: paragraphs, runs, text (tab/break), direct run properties (bold/italic/underline/strike/size/RGB), paragraph formatting (alignment/indentation/spacing), styles (with `basedOn`), numbering (`numPr`), body section geometry, media references, inline drawings (embedded pictures → media-referencing drawing nodes), hyperlinks (external/internal, wrapping their runs), tables (grid/rows/cells with nested block content, `gridSpan`/`vMerge` merge geometry, nested tables), and fields (simple `w:fldSimple` and complex `fldChar` sequences → instruction + cached result) — everything unmapped dispositioned in a deterministic compatibility report (no silent loss) |
