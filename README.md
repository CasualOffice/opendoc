...
| Semantic DOCX import | `.docx` → v1 model: paragraphs, runs, text (tab/break), direct run properties (bold/italic/underline/strike/size/RGB), paragraph formatting (alignment/indentation/spacing), styles (with `basedOn`), numbering (`numPr`), body section geometry, media references, inline drawings (embedded pictures → media-referencing drawing nodes), hyperlinks (external/internal, wrapping their runs), tables (grid/rows/cells with nested block content, `gridSpan`/`vMerge` merge geometry, nested tables), fields (simple `w:fldSimple` and complex `fldChar` sequences → instruction + cached result), and text boxes (`w:txbxContent` in DrawingML or VML → an inline box holding block content, with `mc:AlternateContent` branch selection) — everything unmapped dispositioned in a deterministic compatibility report (no silent loss) |
